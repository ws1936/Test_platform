...
| GET | `/cases/{case_id}` | 用例详情 | 是 |
| PUT | `/cases/{case_id}` | 更新用例 | 是 |
| DELETE | `/cases/{case_id}` | 删除用例 | 是 |
| POST | `/cases/{case_id}/run` | 执行单用例 | 是 |

### 3.8 执行与报告

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/projects/{project_id}/runs` | 创建执行批次 | 是 |
| GET | `/projects/{project_id}/runs` | 执行历史 | 是 |
