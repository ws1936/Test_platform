...
| F023 | Test Generator（消费 Intent 生成用例） | P1 | Done | 7 阶段流程图第 4 步：把 TestIntent 转为 `TestCaseCreateRequest`。扩展 F012/F013 的 service 层：增加 `?design=schema`（策略驱动，默认 `"simple"` 与 F012 字节级一致）/ `?design=simple` 切换；自动生成多类型断言：status_code / json_path（基于 response schema required 字段）/ header（content-type 等）。**配置项 0 新增**（复用 F022 `generator_max_intents_per_operation=20`）；越界返回 **400** `GENERATOR_INTENT_LIMIT_EXCEEDED`（沿用 F012/F013 §5.1 范式；拒绝 BACKLOG 原文 422 提议）。Batch 模式超限整批 abort（不复用 F013 per-doc 隔离——intent 超限属平台保护而非数据错误）。json_path 缺失响应 schema 时静默跳过。鉴权沿用 F012 `_load_project_suite`。新增 `src/app/domain/test_generator/`（`generator.py` 纯函数 + `__init__.py`）+ `src/tests/test_test_generator.py`（13 单测 + 2 defensive）。改造 `src/app/domain/openapi_importer/service.py`（4 方法均接受 `design: Literal["simple","schema"]`，`design="simple"` 字节级 = F012）、`openapi_importer_router.py`（加 `?design=` Query + Literal 校验）、`openapi_importer/exceptions.py`（新 `OpenApiIntentLimitExceededError`）、`openapi_importer/schema.py`（响应模型加可选 `strategy` / `total_intents`）。增量 `src/tests/test_openapi_importer.py`（8 集成测 FT-S/R）+ `src/tests/test_authz_regressions.py`（3 鉴权测 FT-A）。`src/tests/conftest.py` 增量 6 高频 fixture（auth_headers / admin_user / member_user / make_project / make_environment / make_suite）。文档同步：`docs/01-product/F023_SPEC.md`（QA Proposal → Draft v1）/ `docs/04-rules/ADR.md` ADR-009（Proposed → Accepted 2026-09-23）/ `docs/03-api/OPENAPI.yaml`（加 `?design=` Query）/ `docs/03-api/ERROR_CODE.md §5.1`（新 `GENERATOR_INTENT_LIMIT_EXCEEDED` 行）/ `docs/05-test/ACCEPTANCE.md §4.8`（F023 验收）/ `docs/02-design/ARCHITECTURE.md §4.3`（F012→F013→F021→F022→F023 链路图）。全量 `pytest src/tests -v` **509/509 全绿**（480 baseline + 29 F023），0 退化。零 DB Migration、零 model.py 改动、零新依赖、零新配置项。
| F024 | 导入向导 v2（前端子子集 + 预览增强） | P1 | Done | 7 阶段流程图 4 步配套 UI：`WorkspaceImport.tsx` 增加生成模式 Radio（`simple` 默认 / `schema`）+ Preview 面板在 schema 模式下展开 per-intent 行（含 Strategy Tag、Checkbox 选择、全选/全不选按钮）+ 顶部 Statistics 多展示"总计 intent / 本次将创建"指标。前端 `frontend/src/api/openApiImport.ts` 增加 `previewSchema` / `commitSchema` 两个方法（带 `DesignMode = Literal["simple","schema"]` 类型）。`frontend/src/api/types.ts` 给 `OperationPreview.strategy?: string \| null` 与 `ImportPreview.total_intents?: number \| null` 加可选字段（向后兼容）。零后端改动 — 复用 F023 后端 `?design=` Query。`npm run check`（lint + typecheck + build）全绿。零前端自动化测试（KISS）；前端 `frontend/README.md` 已建。设计稿 `docs/01-product/F024_SPEC.md` 待补；架构标注待补。
...
F022 Test Design Engine   [Done 2026-08-20]
   ↓
F023 Test Generator   [Done 2026-09-23]
   ↓
F024 导入向导 v2   [Done 2026-09-23]
```
...
- ✅ **ADR-009**（2026-09-23，Accepted）：F023 决策集——`?design=simple|schema` Query / 错误码 `GENERATOR_INTENT_LIMIT_EXCEEDED` 走 400 范式（拒绝 BACKLOG 422 提议）/ 复用 F022 `generator_max_intents_per_operation`（**0 新增配置项**）/ batch 整批 abort / json_path 静默跳过 / 鉴权沿用 F012
